...
export class WishChartFreqComponent {
  ...
  ngOnInit(): void {
    ...
    var i = 0
    data.forEach(d => {
      let series = this.chart?.area(anychart.data.set(d));
      series?.id(i++)
    })
  ...
  private formattedPulls() {
    ...
    let minmax = this.findMinMax(pulls);
    let groupedByDate = this.groupByRankType(pulls)
      .map(pulls => this.groupByDate(pulls[1]))
      .map(dicts => {
        let arr = Object.entries(dicts);
        var daysOfYear = []
        for (var d = new Date(minmax[0].time); d <= new Date(minmax[1].time); d.setDate(d.getDate() + 1)) {
          let findDate = arr.find(e => e[0] === formatDate(d, 'yyyy-MM-dd', 'en'));
          if (findDate) {
  ...
  // private groupByDate(pulls: Pull[]) {
  //   return Object.entries(groupBy(pulls.map(pull => {
  //     pull.time = formatDate(pull.time, 'yyyy-MM-dd', 'en')
  //     return pull
  //   }), pull => pull.time))
  //     .map(pulls => ({value: pulls[1].length, x: pulls[0]}))
  // }

  private findMinMax(pulls: Pull[]) {
    var temp = pulls.map(pull => {
      pull.time = formatDate(pull.time, 'yyyy-MM-dd', 'en')
      return pull
